...
//  } else { audio.volume(0); }
  ...
  if (p) {
    for (int count = 0; count < 500; count++)
    {   
      orbitArray_red[count].setOrbitColor(COLORS[(int)random(0, 5)]); 
      orbitArray_red[count].orbit();  
    }
    
    for (int count = 0; count < 100; count++)
    {
      if (orbitArray_cyan[count].timer.check(3000)) {
         orbitArray_cyan[count].setOrbitColor(COLORS[(int)random(0, 5)]); 
         orbitArray_green[count].setOrbitColor(COLORS[(int)random(0, 5)]);
         orbitArray_cyan[count].timer.reset(); 
      }
  ...
  // Save images
//  String timestamp = year() + nf(month(),2) + nf(day(),2) + "-"  + nf(hour(),2) + nf(minute(),2) + nf(second(),2) + nf(millis(), 2);
//  if (!killswitch.check(30000)) saveFrame("img-" + timestamp + ".jpg");
}
